...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!iso) return '';
  // Obtener sólo la parte fecha (acepta 'YYYY-MM-DD' o 'YYYY-MM-DDTHH:MM:SS...') y evitar parsing UTC
  const datePart = String(iso).split('T')[0].split(' ')[0];
  const parts = datePart.split('-').map(Number);
  if (parts.length === 3) {
    const [yyyy, mm, dd] = parts;
    const d = new Date(yyyy, mm - 1, dd); // crea fecha en zona local
    const day = String(d.getDate()).padStart(2, '0');
    const month = String(d.getMonth() + 1).padStart(2, '0');
    const year = d.getFullYear();
    return `${day}/${month}/${year}`;
  }
  // Fallback: intenta parsear normalmente y formatear en local
  ...
  function render() {
    ...
    const rows = deudores.map(d => {
      // crear mapa mes -> mensualidad
      const mapa = new Map((d.mensualidades || []).map(m => [m.mes.toLowerCase(), m]));

      const celdasMeses = meses.map(m => {
        const idx = monthIdx(m);
        if (idx > idxActual) return `<td class="no-vencido">-</td>`;
        const men = mapa.get(m.toLowerCase());
        if (!men) return `<td class="no-config">-</td>`; // no hay mensualidad definida
        if (men.pagado) {
          return `<td class="pagado">${formatDateISO(men.fecha_pago)}</td>`;
        } else {
          // return `<td class="pendiente">Pendiente${men.fecha_limite}</td>`;
          return `<td class="pendiente">Pendiente</td>`;
        }
      });
